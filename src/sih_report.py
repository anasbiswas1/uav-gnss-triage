...
def main():
    ...
    t2 = e0.rename(columns={'tag': 'Split', 'acc': 'Accuracy', 'macro_f1': 'Macro-F1', 'ece': 'ECE', 'nll': 'NLL', 'brier': 'Brier'})
    t2.to_csv(T / 't2_leakage_audit.csv', index=False)
    md.append('## Table 2. Leakage audit (window level, same detector)\n\n' + md_table(t2))
    fig, ax = plt.subplots(figsize=(4.2, 2.6))
    x = np.arange(3); w = 0.36
    for i, (_, r) in enumerate(e0.iterrows()):
    ...
    g = fm.groupby('aggregation')
    t4 = pd.DataFrame({'Aggregation': [LABEL[a] for a in g.groups], 'Flight accuracy': [ms(v['flight_acc']) for _, v in g],
                       'Flight macro-F1': [ms(v['flight_macro_f1']) for _, v in g], 'AURC': [ms(v['aurc']) for _, v in g],
                       'Coarse accuracy': [ms(v['coarse_acc']) for _, v in g], 'Coarse macro-F1': [ms(v['coarse_macro_f1']) for _, v in g]})
    t4.to_csv(T / 't4_flight_level.csv', index=False)
    md.append('## Table 4. Flight-level verdicts, mean ± std over 5 repetitions (20 train / 20 calibration flights per family)\n\n' + md_table(t4))

    # ---------------------------------------------------------------- T5 conformal
    cf = pd.read_csv(R / 'e1_conformal.csv')
    agg = cf.groupby(['aggregation', 'level', 'alpha', 'family'])[['coverage', 'avg_set_size', 'abstain_rate', 'empty_rate']].mean().reset_index()
    agg['aggregation'] = agg['aggregation'].map(LABEL); agg['family'] = agg['family'].map(LABEL)
    t5 = agg.rename(columns={'aggregation': 'Aggregation', 'level': 'Level', 'alpha': 'alpha', 'family': 'Class',
                             'coverage': 'Coverage', 'avg_set_size': 'Mean set size', 'abstain_rate': 'Abstention', 'empty_rate': 'Empty sets'})
    t5.to_csv(T / 't5_conformal.csv', index=False)
    md.append('## Table 5. Class-conditional conformal prediction sets at the flight level, mean over 5 repetitions\n\n' + md_table(t5))
    sub = agg[(agg.aggregation == 'Stacked') & (agg.alpha == 0.1)]
    ...
    for ax, lvl, classes in ((axes[0], 'fine', CLASSES), (axes[1], 'coarse', COARSE)):
        ...
        ax.bar(x - w / 2, d['coverage'], w, label='Coverage'); ax.bar(x + w / 2, d['abstain_rate'], w, label='Abstention')
    ...
    t6 = pd.DataFrame({'Held-out family': lo['held_out_family'].map(LABEL), 'Held-out subtype': lo['held_out_subtype'],
                       ...
                       'Fine abstention (0.10)': lo['abstain_a0.1'], 'Coarse coverage (0.10)': lo['coarse_coverage_a0.1'],
                       'Coarse abstention (0.10)': lo['coarse_abstain_a0.1'], 'Predicted as': lo['pred_dist_on_held']})
    ...
    ax.legend(frameon=False, ncol=5, fontsize=7, loc='lower center', bbox_to_anchor=(0.5, 1.0))
        ...
        try:
            ...
            for f in sorted(pathlib.Path(args.whelan_dir).glob('*.ulg')):
                ...
                rows.append({'Log': f.name.split('-')[1].capitalize(), 'fix_type min': int(np.min(g.data['fix_type'])),
                             'satellites min': int(np.min(g.data['satellites_used'])), 'noise max': int(np.max(g.data['noise_per_ms'])),
                             'jamming indicator max': int(np.max(g.data['jamming_indicator'])), 'eph max (m)': float(np.max(g.data['eph'])),
